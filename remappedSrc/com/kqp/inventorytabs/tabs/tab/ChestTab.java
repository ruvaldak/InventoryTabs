...
public class ChestTab extends SimpleBlockTab {
    ...
    public ItemStack getItemFrame() {
        ...
        BlockPos doubleChestPos = ChestUtil.isDouble(world, blockPos) ? getOtherChestBlockPos(world, blockPos)
                : blockPos;
        Box box = new Box(blockPos.toCenterPos(), doubleChestPos.toCenterPos());
        double x = box.minX;
        double y = box.minY;
        double z = box.minZ;
        double x1 = box.maxX;
        double y1 = box.maxY;
        double z1 = box.maxZ;
        List<ItemFrameEntity> list1 = world.getNonSpectatingEntities(ItemFrameEntity.class,
                new Box(x - 0.8, y, z, x1 + 1.8, y1 + 0.8, z1 + 0.8));
        List<ItemFrameEntity> list2 = world.getNonSpectatingEntities(ItemFrameEntity.class,
                new Box(x, y, z - 0.8, x1 + 0.8, y1 + 0.8, z1 + 1.8));
        List<ItemFrameEntity> list3 = world.getNonSpectatingEntities(ItemFrameEntity.class,
                new Box(x, y - 0.8, z, x1 + 0.8, y1 + 1.8, z1 + 0.8));
