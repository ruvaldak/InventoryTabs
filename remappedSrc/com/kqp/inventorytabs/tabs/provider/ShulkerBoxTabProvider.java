...
public class ShulkerBoxTabProvider extends BlockTabProvider {
    ...
    public void addAvailableTabs(ClientPlayerEntity player, List<Tab> tabs) {
        ...
        shulkerTabs.stream().filter(tab -> {
            BlockEntity blockEntity = player.method_48926().getBlockEntity(tab.blockPos);

            if (blockEntity instanceof ShulkerBoxBlockEntity) {
                BlockState blockState = player.method_48926().getBlockState(tab.blockPos);

                return !ShulkerBoxBlockInvoker.invokeCanOpen(blockState, player.method_48926(), tab.blockPos,
